...
from app.modules.controllers.penyandang import accept_invitation, list_pemantau, send_emergency_to_pemantau
...
@router.get("/send-emergency-to-pemantau")
async def send_emergency_handler(
    user_id: int = Depends(auth_middleware),
    _ = Depends(role_middleware(["penyandang"]))
):
    penyandang = await Penyandang.get_or_none(user_id=user_id)
    return await send_emergency_to_pemantau(penyandang_id=penyandang.id)
